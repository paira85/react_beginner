  ...
  return (
    <Card className="w-full h-fit p-4 gap-4 !cursor-pointer" onClick={()=>{navigate(`/topics/${props.id}/detail`)}}>
        <div className="flex items-start gap-4">
            ...
            <img src={props.thumbnail} alt="@THUMBNAIL"  className="w-[140px] h-[150px] asspect-square rounded-lg object-cover"/>
        </div>
        <Separator />
        <div className="w-full flex items-center justify-between">
            <p>{nickname}</p>
            <p>{dayjs(props.created_at).format("YYYY. MM. DD")} - ({dayjs(props.created_at).fromNow()})</p>
        </div>
    </Card>
  )
}
